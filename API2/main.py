...
def Registrar_P(cedula: str, vacuna: str, provincia: str, fecha_v: str):
    ...
    lat = 0.0
    lon = 0.0
    pos = registro.execute("SELECT * FROM PROVINCIAS WHERE Nombre = '"+provincia+"' ")
    pos_d = pos.fetchall()
    for i in pos_d:
        lat = i[2]
        lon = i[3]
    ...
    if datos == []: 
        try:     
            if data['Cedula']==cedula:
                info= (data['Cedula'],data['Nombres'],data['Apellido1'],datetime.datetime.strptime(data['FechaNacimiento'], '%Y-%m-%d %H:%M:%S.%f').date(),vacuna,provincia,fecha_v,signo[mes],lat,lon)
                sql=''' INSERT INTO VACUNADO(CEDULA,NOMBRE,APELLIDO,FECHA_N,VACUNA,PROVINCIA,FECHA_P,SIGNO,LATITUD,LONGITUD) VALUES (?,?,?,?,?,?,?,?,?,?) '''
